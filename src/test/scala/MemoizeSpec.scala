...
class MemoizeSpec extends AnyFlatSpec with must.Matchers {
  ...
  protected def fibb(n: Long): Long =
    n match
      case n if n < 0 =>
  ...
  it must "memoize implicit arguments the same way it does for normal arguments" in {
    ...
    given Int = 5
    val g: Memo[Int, Int => Memo[Int, (Int) ?=> Int]] = memoize(f)
  ...
  it must "memoize a Function0" in {
    val f = () => fibb(40)
    val g = memoize(f)
    val (_, time1) = benchmark(g())
    val (_, time2) = benchmark(g())
    assert(time2 < time1)
  }

  it must "memoize a polymorphic identity function" in {
    val f = memoize([X] => identity[X](_: X))
    f(1) mustEqual identity(1)
    f(1) mustNot equal(identity(2))
  }
  ...
  it must "be significantly faster to compute the fibonacci sequence on large inputs" in {
